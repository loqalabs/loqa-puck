...
func main() {
	...
	log.Printf("🚀 Starting Loqa Puck - Go Reference Implementation")
	...
	client := transport.NewPuckClient(*hubAddr, *puckID)
	...
	go func() {
		for audioChunk := range streamManager.GetPlaybackChannel() {
			...
			if err := puckAudio.PlayAudio(audioData); err != nil {
	...
	// Display status
	fmt.Println()
	fmt.Println("🎤 Loqa Puck - Go Reference Implementation Active!")
	fmt.Println("===============================================")
	fmt.Println()
	fmt.Println("🎙️  Microphone: Listening for wake word")
	fmt.Println("🎯 Wake Word: \"Hey Loqa\" (enabled)")
	fmt.Println("🔊 Speakers: Ready for TTS playback via NATS")
	fmt.Println("📡 Audio Upload: HTTP/1.1 streaming with binary frames")
	fmt.Println("📨 Audio Download: TTS audio via NATS")
	fmt.Println("🔧 ESP32 Ready: 4KB frame limits, optimized protocol")
	fmt.Println()
